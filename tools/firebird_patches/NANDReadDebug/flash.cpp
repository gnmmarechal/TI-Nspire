...
uint8_t nand_read_data_byte() {
    ...
    switch (nand.state) {
        case 0x00:
            ...
            calcd = nand.nand_row * nand.metrics.page_size + nand.nand_col++;
            if(/*lastreadlr != arm.reg[14] &&*/ calcd < 0x4000) {
                printf("Attempt to read manuf offset 0x%X at LR 0x%08X\n",calcd,arm.reg[14]);
                lastreadlr = arm.reg[14];
            }
                return nand_data[calcd];
...
uint32_t nand_read_data_word() {
    ...
    switch (nand.state) {
		...
		if(lastreadlr != arm.reg[14] && calcd < 0x4000) {
			warn("Attempt to read manuf offset 0x%X at LR 0x%08X",calcd,arm.reg[14]);
			lastreadlr = arm.reg[14];
		}
